...
class Mix_conc_design():
    ...
    def get_initial_Na2O_N_TX_XLC(self, R28):
        ...
        return result
    def Mix_result(self, material_params, Mtx,XLC,Na2O,N_TX,Kd):
...
def main():
    ...
    R28 = 50
    Sn = 15
    initial_params = Mix_conc_design.get_initial_Na2O_N_TX_XLC(R28)
    ...
    for x in initial_params:
        ...
        for N in N_params:
            for Kd in Kd_params:
                Mtx = N/N_TX
                if 250<=Mtx<=450:
                    Sn_pred = Mix_conc_design.pred_Sn(Mtx, XLC, Na2O, N_TX, Kd)
                    R28_pred = Mix_conc_design.pred_R(Mtx, XLC, Na2O, N_TX, Kd)
    ...
    for x in result:
        ...
        if (-5<err_Sn<5) and (0<err_R28<5):
            ...
            mix_final.append(Mix_conc_design.Mix_result(material_params, Mtx,XLC,Na2O,N_TX,Kd))
    print("+++++++++++++++++++++++++++++++++")
    for m in mix_final:
        print(m)
